...
const scroller = testimonials.querySelector('.carousel');
...
function scrollToNextItem() {
    if (scroller.scrollLeft < (scroller.scrollWidth - itemWidth - 1000))
...
function slide() {
    if (!nextBtn.addEventListener('click', scrollToNextItem) && !prevBtn.addEventListener('click', scrollToPrevItem)) {
        scrollToNextItem();
    }
}
let timerId = setInterval(() => slide(),3000);
